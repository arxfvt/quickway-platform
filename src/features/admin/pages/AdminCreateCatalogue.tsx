...
const INCREMENT_PRESETS = ['10,000', '25,000', '50,000', '100,000', '250,000', '500,000', '1,000,000']

// Vehicle spec field definitions
const VEHICLE_SPEC_FIELDS: { key: string; label: string; type: 'text' | 'select' | 'number'; options?: string[] }[] = [
  ...
  { key: 'VIN (masked)',   label: 'VIN (masked)', type: 'text' },
]

// Property spec field definitions
const PROPERTY_SPEC_FIELDS: { key: string; label: string; type: 'text' | 'select' | 'number'; showFor?: string[]; options?: string[] }[] = [
  { key: 'Property Type', label: 'Property Type', type: 'select', options: ['Land', 'House', 'Apartment', 'Commercial'] },
  { key: 'Size',          label: 'Size',          type: 'text' },
  { key: 'Location',      label: 'Location',      type: 'text' },
  { key: 'Bedrooms',      label: 'Bedrooms',      type: 'number', showFor: ['House', 'Apartment'] },
  { key: 'Bathrooms',     label: 'Bathrooms',     type: 'number', showFor: ['House', 'Apartment'] },
  { key: 'Tenure',        label: 'Tenure',        type: 'select', options: ['Freehold', 'Leasehold', 'Mailo', 'Kibanja'] },
]
...
function makeLot(): LotDraft {
  return {
    ...
    title: '', description: '', reserve_price: '',
    bid_increment: '100,000', image_url: '', specs: {},
  }
}

// ─────────────────────────────────────────────────────────────────────────────

export default function AdminCreateCatalogue() {
  ...
  useEffect(() => {
    getOrganizations().then((data) => {
      setOrgs(data)
      if (data[0] && !draft.org_id) setDraft((d) => ({ ...d, org_id: data[0].id }))
    }).catch(() => {})
  }, [])
  ...
  const setLotField = (lotId: string, field: keyof LotDraft, val: string) =>
    setDraft((d) => ({
      ...d,
      lots: d.lots.map((l) => l.id === lotId ? { ...l, [field]: val } : l),
    }))

  const setLotSpec = (lotId: string, specKey: string, val: string) =>
    setDraft((d) => ({
      ...d,
      lots: d.lots.map((l) => l.id === lotId ? { ...l, specs: { ...l.specs, [specKey]: val } } : l),
    }))

  // ── Validations ───────────────────────────────────────────────────────
  const step1Valid = draft.org_id.length > 0 && draft.title.trim().length > 0 && draft.location.trim().length > 0
  ...
  // ── Submit ────────────────────────────────────────────────────────────
  const submitCatalogue = async (status: 'scheduled' | 'draft') => {
    setSubmitting(true)
    setSubmitError('')
    try {
  ...
  return (
    <div className="p-6 max-w-[800px] mx-auto">
      <h1 className="text-xl font-bold text-slate-900 mb-1">Create Catalogue</h1>
      <p className="text-xs text-slate-500 mb-6">Complete all steps to publish an asset catalogue for an organisation.</p>
      ...
      <div className="flex items-center gap-2 mb-8">
        {STEPS.map((s, i) => (
          <div key={s} className="flex items-center gap-2">
            ...
            {i < STEPS.length - 1 && <div className="h-px bg-slate-200 w-8" />}
          </div>
        ))}
      </div>
      ...
      {step === 0 && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
          <h2 className="text-sm font-semibold text-slate-800 mb-4">Catalogue Details</h2>
          ...
          <Field label="Catalogue Title *" value={draft.title} onChange={(v) => set('title', v)} placeholder="e.g. UN Mission Vehicle Fleet — 50 Vehicles" maxLen={100} />
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Describe the catalogue, what's included, and any important terms…"
            ...
            <div key={lot.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-3">
                <Field
                  label="Title *"
                  value={lot.title}
                  onChange={(v) => setLotField(lot.id, 'title', v)}
                  placeholder={isVehicles ? 'e.g. 2020 Toyota Land Cruiser' : isProperty ? 'e.g. Kololo Retail Unit' : 'e.g. Lot title'}
                />
                <Field
                  label="Image URL"
                  value={lot.image_url}
                  onChange={(v) => setLotField(lot.id, 'image_url', v)}
                  placeholder="https://…"
                />
                <Field
                  label="Reserve Price (UGX)"
                  value={lot.reserve_price}
                  onChange={(v) => setLotField(lot.id, 'reserve_price', v)}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setLotField(lot.id, 'bid_increment', e.target.value)}
              ...
              <div className="mb-4">
                ...
                <textarea
                  ...
                  onChange={(e) => setLotField(lot.id, 'description', e.target.value)}
                  placeholder="Condition, key details…"
                    ...
                    {VEHICLE_SPEC_FIELDS.map((f) =>
                      f.type === 'select' ? (
                        <div key={f.key}>
                          ...
                          <select
                            value={lot.specs[f.key] ?? ''}
                            onChange={(e) => setLotSpec(lot.id, f.key, e.target.value)}
                            className="w-full px-3.5 py-2.5 rounded-xl text-sm border border-slate-200 bg-white focus:outline-none focus:ring-2 focus:ring-brand/20 focus:border-brand transition-colors"
                          >
                        ...
                        <SpecField
                          ...
                          label={f.label}
                          value={lot.specs[f.key] ?? ''}
                          onChange={(v) => setLotSpec(lot.id, f.key, v)}
                          type={f.type === 'number' ? 'number' : 'text'}
                          placeholder={f.key === 'Year' ? 'e.g. 2020' : f.key === 'Mileage' ? 'e.g. 48,000 km' : ''}
                        />
                      )
                    )}
                  </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {PROPERTY_SPEC_FIELDS.map((f) => {
                      ...
                      if (f.showFor && !isResidential) return null
                      return f.type === 'select' ? (
                        ...
                        <SpecField
                          ...
                          onChange={(v) => setLotSpec(lot.id, f.key, v)}
                          type={f.type === 'number' ? 'number' : 'text'}
                          placeholder={f.key === 'Size' ? 'e.g. 500 sqm or 2 acres' : f.key === 'Location' ? 'e.g. Kololo, Kampala' : ''}
                        />
                      )
                    })}
                  </div>
              ...
              {/* Generic spec rows for other categories */}
              {!isVehicles && !isProperty && (
                <div>
                  <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide mb-2">Specifications</p>
                  <GenericSpecRows specs={lot.specs} onUpdate={(specs) => setDraft((d) => ({ ...d, lots: d.lots.map((l) => l.id === lot.id ? { ...l, specs } : l) }))} />
                </div>
              )}
            </div>
          ))}
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
          ...
          <div>
            ...
            <textarea rows={3} value={draft.bank_details} onChange={(e) => set('bank_details', e.target.value)}
              className="w-full px-3.5 py-2.5 rounded-xl text-sm border border-slate-200 bg-white focus:outline-none focus:ring-2 focus:ring-brand/20 focus:border-brand transition-colors resize-none" />
            <p className="text-[10px] text-slate-400 mt-1">Shown to bidders when they pay the entry fee.</p>
          </div>
      ...
      {step === 3 && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-5">
          ...
          <div className="bg-slate-50 rounded-xl p-4 space-y-2 text-xs">
            <ReviewRow label="Organisation" value={orgs.find((o) => o.id === draft.org_id)?.name ?? draft.org_id} />
          ...
          <p className="text-xs text-slate-500 bg-brand-light border border-brand/20 rounded-xl px-4 py-3 leading-relaxed">
            Publishing will make this catalogue visible in the public auction list. Bidders can register and pay the entry fee to participate.
          </p>
          {submitError && (
            <div className="text-xs text-red-700 bg-red-50 border border-red-200 rounded-xl px-3 py-2.5">
              {submitError}
            </div>
          )}
          <div className="flex gap-3">
            <button
              onClick={() => submitCatalogue('draft')}
              disabled={submitting}
            ...
            <button
              onClick={() => submitCatalogue('scheduled')}
              disabled={submitting}
              className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold bg-brand hover:bg-brand-dark text-white transition-colors disabled:opacity-50"
            >
              {submitting && <Loader2 size={14} className="animate-spin" />}
              {submitting ? 'Publishing…' : 'Publish Catalogue'}
            </button>
          </div>
        </div>
      )}
...
function SpecField({ label, value, onChange, placeholder, type = 'text' }: {
  label: string; value: string; onChange: (v: string) => void
  ...
  return (
    <div>
      ...
      <input
        ...
        onChange={(e) => onChange(e.target.value)}
        placeholder={placeholder}
        className="w-full px-3 py-2 rounded-lg text-sm border border-slate-200 bg-white text-slate-800 focus:outline-none focus:ring-2 focus:ring-brand/20 focus:border-brand transition-colors"
      />
    </div>
  )
}

function GenericSpecRows({ specs, onUpdate }: { specs: Record<string, string>; onUpdate: (s: Record<string, string>) => void }) {
  const entries = Object.entries(specs).length > 0 ? Object.entries(specs) : [['', ''], ['', ''], ['', '']]
  return (
        ...
        <div key={i} className="grid grid-cols-2 gap-2">
          <input
            ...
            onChange={(e) => {
              ...
              onUpdate(Object.fromEntries(newEntries.filter(([key]) => key.trim())))
            }}
            className="px-3 py-2 rounded-lg text-sm border border-slate-200 focus:outline-none focus:ring-2 focus:ring-brand/20 focus:border-brand"
          />
          <input
            placeholder="Value"
            value={v}
